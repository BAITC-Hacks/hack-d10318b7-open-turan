...
def generate_protocol_docx(
    *,
    ...
    date = doc.add_paragraph(f"Дата встречи: {date_text}")
    date.alignment = WD_ALIGN_PARAGRAPH.CENTER

    doc.add_heading("Краткое резюме", level=1)
    doc.add_paragraph(clean_text(summary))

    doc.add_heading("Участники, явно упомянутые в речи", level=1)
    ...
    if clean_decisions:
        ...
        doc.add_paragraph("Явные решения не зафиксированы.")
    ...
    if rows:
        ...
        table.autofit = False
        widths = [Cm(0.8), Cm(8.0), Cm(4.2), Cm(3.2)]
        for column, width in zip(table.columns, widths):
            column.width = width
        headers = ["№", "Задача и подтверждение", "Ответственный", "Срок"]
        ...
        for number, task in enumerate(rows, start=1):
            ...
            if evidence:
                ...
                citation_run = citation.add_run(f"Подтверждение: {evidence}")
                citation_run.italic = True
                citation_run.font.size = Pt(9)
            _keep_row_together(table.rows[-1])
    else:
        doc.add_paragraph("Задач не найдено. При необходимости добавьте их вручную.")

    transcript = clean_text(transcript_text, "")
    if transcript:
        doc.add_page_break()
        doc.add_heading("Полная расшифровка", level=1)
