...
os.environ.setdefault("HF_HOME", str(Path(__file__).resolve().parent / "models" / "hf-cache"))
...
if not WHISPER_CACHE_DIR.is_absolute():
    WHISPER_CACHE_DIR = Path(__file__).resolve().parent / WHISPER_CACHE_DIR
...
def normalize_tasks(rows: object) -> list[dict[str, str]]:
    ...
    for row in rows:
        ...
        if not clean_text(row.get("task"), ""):
            continue
        normalized.append(
            {
    ...
    try:
        ...
        for segment in iterator:
            ...
            if (
                ...
                rejected += 1
                continue
            transcript_length += len(text) + 1
            if transcript_length > MAX_TRANSCRIPT_CHARS:
                raise ProcessingError("Расшифровка длиннее 60 000 символов. Разделите запись на части.")
            accepted.append(
                {
...
def structure_meeting(
    ...
    try:
        ...
        if not isinstance(envelope, dict) or not isinstance(envelope.get("message"), dict):
            raise ValueError("Локальная модель вернула неверный формат ответа.")
...
def ollama_status() -> dict:
    try:
        ...
        response = httpx.get(f"{url}/api/tags", timeout=2.0, trust_env=False)
        response.raise_for_status()
        envelope = response.json()
        if not isinstance(envelope, dict) or not isinstance(envelope.get("models"), list):
            raise ValueError("Неверный ответ Ollama")
        ...
        return {
            "online": True,
            "model_available": expected in installed or OLLAMA_MODEL in installed,
            "model": OLLAMA_MODEL,
        }
